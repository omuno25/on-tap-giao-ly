  ...
  return (
    <main className="min-h-screen bg-surface px-4 pb-28 pt-6">
      ...
      <section className="mt-7 overflow-hidden rounded-feature bg-hero p-6 text-on-hero shadow-soft">
        ...
        <p className="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-on-hero-muted">Giáo lý Hôn nhân</p>
        <h1 className="mt-2 font-headline text-2xl font-bold leading-tight">Bộ câu hỏi Hôn nhân Công giáo</h1>
        <p className="mt-3 text-sm leading-relaxed text-on-hero-muted">Ôn tập kiến thức Hôn nhân và Gia đình Công giáo bằng câu hỏi–đáp án.</p>
      </section>
      ...
      <section className="mt-5 rounded-card border border-surface-container bg-surface-container-lowest p-5">
        ...
        <Link href="/study" className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-primary px-6 py-3.5 font-bold text-on-primary transition active:scale-[0.98]">
          {position > 0 ? "Tiếp tục học" : "Bắt đầu học"}
          <ChevronRight className="size-[var(--icon-sm)]" />
        </Link>
        <Link href="/test" className="mt-3 flex w-full items-center justify-center gap-2 rounded-full border border-primary px-6 py-3.5 font-bold text-primary transition-colors hover:bg-primary/5 active:scale-[0.98]">
          <ClipboardCheck className="size-[var(--icon-sm)]" /> Kiểm tra 20 câu
        </Link>
        {position > 0 && (
          <button type="button" onClick={reset} className="mt-2 flex w-full cursor-pointer items-center justify-center gap-2 px-6 py-3 text-sm font-bold text-on-surface-variant hover:text-primary">
      ...
      <section className="mt-7">
        <h2 className="font-headline text-lg font-bold">Câu hỏi + câu trả lời</h2>
        <div className="mt-3 grid gap-3">
          {questions.map((question, index) => (
            <article key={question.id} className="overflow-hidden rounded-card border border-surface-container bg-surface-container-lowest">
              <div className="flex items-start gap-3 p-4">
                ...
                <h3 className="min-w-0 flex-1 text-sm font-bold leading-relaxed">{question.question}</h3>
              </div>
              <div className="border-t border-surface-container bg-surface-container-low p-4">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-primary">Trả lời</p>
                <p className="whitespace-pre-line text-sm leading-relaxed text-on-surface-variant">{formatAnswer(question.answer)}</p>
              </div>
            </article>
          ))}
        </div>
