...
import { X, Church, RefreshCw, Timer, TrendingUp, Award } from 'lucide-react';
...
export default function StudyMode() {
  ...
  const flashcard = MOCK_QUESTIONS.find(q => q.type === 'flashcard')!;
    ...
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-md flex items-center justify-between px-6 py-3 border-b border-surface-container">
        <div className="flex items-center gap-3">
          <Link href="/" className="hover:bg-surface-container-low p-2 rounded-full transition-colors active:scale-95">
            <X className="w-5 h-5 text-on-surface" />
          </Link>
          <div className="flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant leading-none mb-0.5">Phiên học</span>
            <span className="font-headline font-bold text-primary text-sm leading-tight">Giáo lý Hôn nhân - Bộ 36 câu</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs font-medium text-on-surface-variant">12/36 thẻ</span>
          <div className="w-24 h-1.5 bg-surface-container-highest rounded-full overflow-hidden">
            <div className="h-full w-[33%] bg-gradient-to-r from-secondary to-secondary-container rounded-full" />
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 pt-20 pb-28">
          ...
          <AnimatePresence mode="wait">
            {!isFlipped ? (
              ...
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="grid grid-cols-4 gap-3 p-1.5 bg-surface-container-low rounded-xl"
              >
                {['Lại', 'Khó', 'Tốt', 'Dễ'].map((label, i) => (
                  <button 
                    key={label}
                    className="flex flex-col items-center gap-1.5 p-3 rounded-lg hover:bg-primary-container/20 transition-all active:scale-90 group"
                  >
        ...
        <div className="mt-12 text-on-surface-variant/40 flex items-center gap-6">
          <div className="flex items-center gap-2">
            <kbd className="px-1.5 py-0.5 bg-surface-container-high rounded text-[9px] font-bold">CÁCH</kbd>
            <span className="text-[11px] font-medium">Lật</span>
          </div>
          <div className="flex items-center gap-2">
            <kbd className="px-1.5 py-0.5 bg-surface-container-high rounded text-[9px] font-bold">1-4</kbd>
      ...
      {/* Stats Footer */}
      <section className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-2xl px-6 hidden md:block">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-surface-container-low/50 backdrop-blur-sm p-3 rounded-lg flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[9px] uppercase font-bold text-on-surface-variant/60 tracking-tight leading-none mb-0.5">Thời gian học</p>
          ...
          <div className="bg-surface-container-low/50 backdrop-blur-sm p-3 rounded-lg flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[9px] uppercase font-bold text-on-surface-variant/60 tracking-tight leading-none mb-0.5">Độ chính xác</p>
          ...
          <div className="bg-surface-container-low/50 backdrop-blur-sm p-3 rounded-lg flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[9px] uppercase font-bold text-on-surface-variant/60 tracking-tight leading-none mb-0.5">Chuỗi ngày</p>
