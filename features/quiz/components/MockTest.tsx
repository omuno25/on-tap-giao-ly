...
export default function MockTest() {
  ...
  const question = MOCK_QUESTIONS.find(q => q.type === 'multiple-choice')!;
      ...
      <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-md flex items-center justify-between px-6 py-3 border-b border-surface-container">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-primary tracking-widest font-headline uppercase">Giáo Xứ Chánh Tòa</span>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex flex-col items-end">
            <span className="text-[9px] font-bold text-outline uppercase tracking-tighter">Phòng Thi</span>
            <span className="text-xs font-medium text-on-surface">Kỳ Thi Cuối Khóa</span>
          </div>
          <Image 
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2 bg-surface-container-low p-5 rounded-2xl flex flex-col justify-between gap-3">
            <div className="flex justify-between items-center">
              <span className="font-headline font-bold text-on-surface text-sm">
                Câu hỏi 12<span className="text-outline font-normal"> / 36</span>
              </span>
          ...
          <div className="bg-surface-container-low p-5 rounded-2xl flex flex-col items-center justify-center gap-1.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-headline font-black text-on-surface tabular-nums text-lg">14:28</span>
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
          {question.options?.map((option) => {
            ...
            return (
              ...
              >
                <span className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full font-headline font-bold text-xs transition-colors ${
                  isSelected 
                ...
                <span className={`font-medium leading-relaxed text-sm ${isSelected ? "text-on-primary-container" : ""}`}>
                  {option.text}
                </span>
                {isSelected && (
