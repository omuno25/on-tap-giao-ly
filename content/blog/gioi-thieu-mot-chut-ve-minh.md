---
title: "Giới thiệu một chút về mình"
category: "Giới thiệu"
excerpt: "Một vài dòng về người xây dựng FlashCard và lý do ứng dụng nhỏ này được bắt đầu."
readingTime: "2 phút đọc"
featured: false
order: 0
---

Chào bạn, mình là người xây dựng **FlashCard** — một ứng dụng nhỏ giúp việc học Giáo lý và các kinh cần thuộc trở nên đơn giản, rõ ràng hơn trên điện thoại.

## Vì sao mình làm ứng dụng này?

Mình bắt đầu dự án từ một nhu cầu rất thực tế: nội dung cần học khá nhiều, nhưng nếu được chia thành từng thẻ ngắn thì sẽ dễ xem lại và ghi nhớ hơn. Mình muốn tạo ra một nơi có thể mở lên học ngay, không cần đăng ký tài khoản và không bị phân tâm bởi những chức năng không cần thiết.

FlashCard hiện tập trung vào:

- Giáo lý Hôn nhân.
- Giáo lý Dự tòng.
- Các kinh cần thuộc kèm audio.
- Bài kiểm tra và thống kê tiến độ học.

## Một dự án mở

Đây vẫn là một dự án đang được hoàn thiện từng ngày. Nội dung và tính năng sẽ tiếp tục được bổ sung dựa trên nhu cầu sử dụng thực tế.

Mình dự định mở mã nguồn để mọi người có thể xem, đóng góp và cùng cải thiện ứng dụng. Hy vọng FlashCard sẽ giúp bạn học đều đặn hơn, dù mỗi ngày chỉ dành ra một chút thời gian.
